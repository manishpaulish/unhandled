...
**OCaml 5 will not tell you which effects a library requires you to handle.
This works it out from the compiler's own typed trees.**
...
[`docs/ECOSYSTEM-CONTRACTS.md`](docs/ECOSYSTEM-CONTRACTS.md) is that output for
...
On real code it is less settled: across 39 repositories it has reported 31
escapes and **every one was our own false positive**, 25 since fixed and 6
documented in [`docs/LIMITATIONS.md`](docs/LIMITATIONS.md). See
[`bench/NEGATIVE-RESULT.md`](bench/NEGATIVE-RESULT.md) for the whole account.
